...
"""
    ...
    def get_system_prompt(cls) -> Prompt:
        ...
        return Prompt(template=cls.system_prompt_template, input_variables=input_variables, content=system_prompt)
    ...
    def get_prompt(cls, document: CleanedDocument) -> GenerateDatasetSamplesPrompt:
        ...
        prompt_template = PromptTemplate.from_template(template=cls.prompt_template_str, template_format="jinja2")
    ...
    @classmethod
    def generate(
        cls, prompts: dict[DataCategory, list[GenerateDatasetSamplesPrompt]], test_size: float = 0.2, mock: bool = False
    ) -> TrainTestSplit:
        ...
        def _to_langchain(prompt: GenerateDatasetSamplesPrompt) -> list[BaseMessage]:
            messages = [SystemMessage(content=cls.get_system_prompt().content), HumanMessage(content=prompt.content)]
...
def get_dataset_generator(dataset_type: DatasetType) -> type[DatasetGenerator]:
    ...
    if dataset_type == DatasetType.PREFERENCE:
